...
export async function GET(request: NextRequest) {
  try {
    ...
    if (sheet === "Data RT") {
      ...
      rows = (await readCollection("announcements")).map((d:any)=>({"Firestore ID":d.id,"Judul":text(d.title),"Isi Singkat":text(d.summary),"URL Lampiran":text(d.attachmentUrl),"Prioritas":text(d.priority),"Berlaku Sampai":dateOut(d.validUntil),"Urutan":Number(d.order||0),"Status Aktif":yesNo(d.isActive)}));
    } else if (sheet === "Kegiatan") {
      const posts = await readCollection("posts"); const anns = await readCollection("announcements");
      rows = [...posts.map((d:any)=>({"Firestore ID":d.id,"ID Konten":d.id,"Tanggal":dateOut(d.publishedDate),"Jenis":d.category==="Masyarakat"?"Kegiatan":"Berita","Judul":text(d.title),"Ringkasan":text(d.summary),"Lokasi":text(d.location),"Penanggung Jawab":text(d.authorName),"Status Publikasi":d.status==="published"?"Dipublikasikan":"Draft","Link Foto":text(d.coverImageUrl),"Link Berita":"","Tanggal Publikasi":dateOut(d.publishedDate),"Keterangan":""})), ...anns.map((d:any)=>({"Firestore ID":d.id,"ID Konten":d.id,"Tanggal":"","Jenis":"Pengumuman","Judul":text(d.title),"Ringkasan":text(d.summary),"Lokasi":"","Penanggung Jawab":"","Status Publikasi":d.isActive===false?"Draft":"Dipublikasikan","Link Foto":"","Link Berita":text(d.attachmentUrl),"Tanggal Publikasi":"","Keterangan":""}))];
