...
async function syncPosts(rows: SheetRow[], source = "Berita") {
  const docs = rows.filter((r) => text(r["Judul"])).map((r, i) => {
    ...
    return { id: text(r["Firestore ID"]) || docId("post", r["ID Konten"] || slug, i), data: {
...
async function syncAnnouncements(rows: SheetRow[], source = "Pengumuman") {
  const docs = rows.filter((r) => text(r["Judul"])) .map((r, i) => ({ id: text(r["Firestore ID"]) || docId("announcement", r["Judul"], i), data: {
    title: text(r["Judul"]), summary: text(r["Isi Singkat"] || r["Ringkasan"]), attachmentUrl: text(r["URL Lampiran"] || r["Link Berita"]),
...
async function syncAgendas(rows: SheetRow[]) {
  const docs = rows.filter((r) => text(r["Nama Kegiatan"] || r["Nama Agenda"] || r["Judul"])) .map((r, i) => ({ id: text(r["Firestore ID"]) || docId("agenda", r["ID Agenda"] || r["Nama Kegiatan"] || r["Nama Agenda"], i), data: {
...
async function syncServices(rows: SheetRow[]) {
  const docs = rows.filter((r) => text(r["Nama Layanan"])) .map((r, i) => ({ id: text(r["Firestore ID"]) || docId("service", r["Slug"] || r["Nama Layanan"], i), data: {
...
async function syncDocuments(rows: SheetRow[]) {
  const docs = rows.filter((r) => text(r["Judul Dokumen"])) .map((r, i) => ({ id: text(r["Firestore ID"]) || docId("document", r["Judul Dokumen"], i), data: {
...
async function syncHero(rows: SheetRow[]) {
  const docs = rows.filter((r) => text(r["Judul"])) .map((r, i) => ({ id: text(r["Firestore ID"]) || docId("hero", r["Judul"], i), data: {
...
async function syncGalleryAlbums(rows: SheetRow[]) {
  const docs = rows.filter((r) => text(r["Judul Album"])) .map((r, i) => ({ id: text(r["Firestore ID"]) || docId("gallery", r["Slug"] || r["Judul Album"], i), data: {
...
async function syncUmkm(rows: SheetRow[]) {
  const docs = rows.filter((r) => text(r["Nama Usaha"])) .map((r, i) => ({ id: text(r["Firestore ID"]) || docId("umkm", r["ID UMKM"] || r["Nama Usaha"], i), data: {
    name: text(r["Nama Usaha"]), ownerName: text(r["Nama Pemilik"]), businessType: text(r["Jenis Usaha"]), mainProduct: text(r["Produk Utama"]), address: text(r["Alamat"]),
    ...
    isPublic: r["Tampil Website"] === undefined ? true : yes(r["Tampil Website"]), order: numberValue(r["Urutan"]) || i + 1,
  }}));
  return upsertDocs("umkm", "UMKM", docs);
}

async function syncFacilities(rows: SheetRow[]) {
  const docs = rows.filter((r) => text(r["Nama Fasilitas"])) .map((r, i) => ({ id: text(r["Firestore ID"]) || docId("facility", r["ID Fasilitas"] || r["Nama Fasilitas"], i), data: {
    ...
    condition: text(r["Kondisi"]), manager: text(r["Pengelola"]), status: text(r["Status"]), imageUrl: text(r["Foto/Link"]), isPublic: r["Tampil Website"] === undefined ? true : yes(r["Tampil Website"]), order: numberValue(r["Urutan"]) || i + 1,
  }}));
...
async function syncServiceRequests(rows: SheetRow[]) {
  ...
  for (const r of rows) {
    ...
    const now = new Date().toISOString();
    await setDocument("serviceRequests", id, { ticketId, name: text(r["Nama Pemohon"]), nik: text(r["NIK"]).replace(/\D/g, ""), rt: normalizeRt(r["RT"]),
      ...
      isPublicVerification: yes(r["Tampil Cek Publik"]), syncSource: "Surat", syncedAt: now, updatedAt: now }, true); written++;
  }
  return { written };
}
...
async function syncComplaints(rows: SheetRow[]) {
  ...
  for (const r of rows) {
    ...
    const id = text(r["Firestore ID"]) || ticketId;
    const now = new Date().toISOString();
    await setDocument("complaints", id, { ticketId, name: text(r["Nama Pelapor"]), phone: text(r["Kontak"]), rt: normalizeRt(r["RT"]), category: text(r["Kategori"]), message: text(r["Isi Ringkas"]), location: text(r["Lokasi"]),
      ...
      syncSource: "Pengaduan", syncedAt: now, updatedAt: now }, true); written++;
  }
  return { written };
}
