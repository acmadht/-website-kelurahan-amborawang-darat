      ...
      <main className={styles.page}>
          ...
          <div className={`container ${styles.heroGrid}`}>
            <Reveal enabled={settings.animationEnabled}>
              <div className={styles.heroCopy}>
                ...
                <p>Pusat arsip dokumen Kelurahan {settings.villageName} untuk administrasi, profil, peraturan, dan laporan publik.</p>
        ...
        <section className={styles.introSection}>
          <div className={`container ${styles.introGrid}`}>
            <Reveal enabled={settings.animationEnabled}><div className={styles.introAside}><span className={styles.sectionNumber}>01</span><span className={styles.eyebrow}>Dokumen Publik</span><h2>Informasi yang mudah ditemukan</h2></div></Reveal>
            <Reveal enabled={settings.animationEnabled} delay={60}><div className={styles.introArticle}><p className={styles.lead}>Dokumen publik disusun agar masyarakat dapat mengakses informasi penting tanpa harus mencari berkas secara terpisah.</p><p>Setiap dokumen menampilkan kategori, tahun, dan format berkas agar lebih mudah dikenali sebelum dibuka atau diunduh.</p><div className={styles.introCallout}><span>Catatan</span><strong>Pastikan file yang dipublikasikan telah melalui verifikasi dan merupakan dokumen yang boleh diakses publik.</strong></div></div></Reveal>
          </div>
        </section>

        <section id="daftar-dokumen" className={styles.documentsSection}>
          <div className="container">
            <Reveal enabled={settings.animationEnabled}><div className={styles.documentsHeading}><div><span className={styles.eyebrowLight}>Daftar Dokumen</span><h2>Arsip tersedia</h2></div><div className={styles.documentsCount}><strong>{String(filteredDocuments.length).padStart(2, "0")}</strong><span>dokumen ditampilkan</span></div></div></Reveal>
              ...
              {filteredDocuments.map((item, index) => (
                <Reveal key={item.id} enabled delay={(index % 6) * 35}>
                  <article className={styles.documentCard}>
                    ...
                    <div className={styles.documentBody}><span>{item.category}</span><h3>{item.title}</h3>{item.description ? <p>{item.description}</p> : null}</div>
